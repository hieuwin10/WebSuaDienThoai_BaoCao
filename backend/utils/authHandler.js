        ...
        try {
            ...
            if (req.cookies[JWT_COOKIE_NAME]) {
                ...
                if (!authHeader || !/^Bearer\s+/i.test(authHeader)) {
                    res.status(401).json({ message: 'Bạn chưa đăng nhập hoặc thiếu mã xác thực.' })
                    return;
                }
            ...
            if (!getUser) {
                res.status(401).json({ message: 'Tài khoản không còn hợp lệ. Vui lòng đăng nhập lại.' })
                return;
            }
            req.user = getUser;
            next();
        } catch (error) {
            res.status(401).json({ message: 'Mã đăng nhập không hợp lệ hoặc đã hết hạn.' })
        }
